...
const storeIcon = L.divIcon({
  className: 'store-pin',
  html: `<div class="w-8 h-8 rounded-full bg-orange-600 border-2 border-white shadow-lg flex items-center justify-center text-white text-xs">🏪</div>`,
  iconSize: [32, 32],
  iconAnchor: [16, 16]
});
    ...
    <div className="bg-white rounded-3xl border border-stone-200 overflow-hidden shadow-2xl p-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-stone-100">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-amber-50 border border-amber-200 flex items-center justify-center text-orange-600">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-stone-500 font-medium">Claim #{claim?._id?.slice(-6) || 'ACTIVE'}</span>
            </div>
            <h3 className="text-lg font-bold text-stone-900">{claim?.listingId?.foodName || 'Surplus Food Batch'}</h3>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
        <div className="flex items-center gap-2 text-xs text-stone-500">
          ...
          <span>Pickup Address: <strong>{claim?.listingId?.pickupAddress || 'Connaught Place, New Delhi'}</strong></span>
        </div>
